    ...
    def FireMeasure(self, data): 
        ...
        for y in range (self.ROW):
            for x in range (self.COL): 
                ...
                if data[y][x] > 160 and data[y][x] < 169: 
                    burnt_cells.append(data[y][x])
        fire_size = len(fire_cells)    # Number of cells on fire represents fire size 
        burnt_size = len(burnt_cells)  # Number of cells burnt represents burnt area 
        cost = (fire_size+burnt_size) * unit_cost 
        return fire_size, burnt_size, cost
...
for t in range (0, time):
    ...
    fire_size, burnt_size, cost = Land1.FireMeasure(AA)
    print('The size of fire is', fire_size, 'cells. The burnt area is',  burnt_size, 'The cost associated is', cost, 'USD.')
